...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  const daysWithoutDiscount = numDays % BILLABLE_DAYS_IN_MONTH;
  const daysWithDiscount = numDays - daysWithoutDiscount;
  const discountedPrice =
    dayRate(ratePerHour * (1 - discount)) * daysWithDiscount;
  const undiscountedPrice = dayRate(ratePerHour) * daysWithoutDiscount;
  return Math.ceil(undiscountedPrice + discountedPrice);
}
